...
q.split("").join("\\")+")(.*?)(\\"+v.split("").join("\\")+")","g")}function B(a){return a.split(new RegExp("("+q.split("").join("\\")+")(.*?)("+v.split("").join("\\")+")","g")).map(function(a,b,f){return f[b-1]===q?q+a+v:a}).filter(function(a,b,f){return 0!==a.length&&a!==q&&a!==v&&0!==a.length})}function C(a){return a.replace(new RegExp("["+q+v+"]","g"),"").replace(new RegExp("\\"+r.split("").join("\\")+"(.*)"),"")}function H(a){return a.reduce(function(a,b){b.match(/(\()(.*?)(\))/)?a.vmFilters[a.vmFilters.length]=
b.replace(/[\(\)]/g,""):b.match(/(\[)(.*?)(\])/)?-1!==b.indexOf("~")?a.model[a.model.length]=b.replace(/[\[\]\~]/g,""):-1!==b.indexOf("+")?a.local[a.local.length]=b.replace(/[\[\]\+]/g,""):a.session[a.session.length]=b.replace(/[\[\]\-]/g,""):a.filters[a.filters.length]=b;return a},{filters:[],vmFilters:[],model:[],local:[],session:[]})}function D(a){return-1!==a.indexOf("for")?a.replace(new RegExp("["+q+v+"]","g"),"").replace(/\|(.*)/,"").replace(/\s/g,"").split(/for(.*?)loop/).filter(function(a){return 0!==
...
this._data.innerHTML)):(this.local.stopChange()[this.localAttr]=E(this.bindText),this.isInput&&-1!==["value","checked"].indexOf(this.attr)&&(a=this._data.get(this.key),this.node.stopChange()[this.attr]=void 0!==a?a:"")));return this}),reconnect:h(function(){"for"!==this.type?(this._data.getLayer(this.key).removeDataUpdateListener(this.localKey,this.dataListener),1===this.bindText.length&&this.node.removeAttrUpdateListener(this.listener,this.domListener)):data.getLayer(this.key).removeDataMethodUpdateListener(this.extraListener);
