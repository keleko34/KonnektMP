...
m;b++)this.element.appendChild(this._value[b])}else this.bindTarget[this.bindProperty]=M(this,this.bindText),this.isInput&&(this.element.stopChange()[this.bindTarget.name]=M(this,this.bindText));return this}function U(b){this._data=b;this.filters=void 0!==b.filters?b.filters:this.filters;b=0;for(var m=this.bindNames.length;b<m;b++){var d=this.bindNames[b],a=this.bindMaps[b];a._data.addDataUpdateListener&&(a._data.exists(d)||(console.warn("No property by the name %o exists on this data set %o",d,a._data),
a._data.add(d,a._value)),a.value=a._data.get(d),"component"!==a.type&&"for"!==a.type?(a.isConnected=!0,a._data.addDataUpdateListener(d,a.updateDom)):"for"===a.type&&a._value.addDataUpdateListener&&a._value.addDataUpdateListener("*",a.updateLoop).addDataCreateListener(a.updateLoop).addDataDeleteListener(a.updateLoop))}1!==this.bindText.length||"text"!==this.type&&"attribute"!==this.type&&!this.isEvent||this.element.addAttrUpdateListener("text"===this.type?"html":this.bindListener,this.updateData);
